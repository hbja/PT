...
class MOTSTest(Dataset):
    SEQ_IDS_TEST = ["%04d" % idx for idx in [8]]
    TIMESTEPS_PER_SEQ_TEST = {"0008": 321}
...
class MOTSCarsVal(Dataset):
    ...
    def __init__(self, root_dir='./', type="training", class_id=26, size=None, transform=None, batch=False, batch_num=8):
...
class MOTSTrackCarsValOffset(Dataset):
    SEQ_IDS_TRAIN = ["%04d" % idx for idx in [0, 1, 2, 3, 4, 5]]
    SEQ_IDS_VAL = ["%04d" % idx for idx in [6, 7]]
    TIMESTEPS_PER_SEQ = {"0000": 70, "0001": 168, "0002": 118, "0003": 150, "0004": 485, "0005": 150, "0006": 100, "0007": 102}
    SEQ_IDS_TEST = ["%04d" % idx for idx in [8]]
    TIMESTEPS_PER_SEQ_TEST = {"0008": 321}
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, az=False,
        ...
        if not test:
            ...
            self.mots_root = os.path.join(rootDir, 'car_SE_val_prediction')
        else:
            ids = self.SEQ_IDS_TEST
            timestamps = self.TIMESTEPS_PER_SEQ_TEST
            self.image_root = os.path.join(kittiRoot, 'testing/image_02/')
            self.mots_root = os.path.join(rootDir, 'car_SE_test_prediction')
